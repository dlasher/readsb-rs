...
#[test]
fn test_compare_with_output_file() {
    fn make_beast_frame(payload: &[u8]) -> Vec<u8> {
        let mut buf = Vec::new();
        buf.push(0x1a); buf.push(0x32);
        buf.extend_from_slice(&[0u8; 6]);
    ...
    let output_path = "/tmp/test_compare_with_output_file.txt";
    ...
    let output = beast_client()
        .args(["compare",
               ...
               "--window", "1", "--duration", "3",
               "--output", output_path])
        .output().expect("Failed to run compare with --output");

    assert!(output.status.success(),
        "compare with --output should succeed: stderr={}",
        String::from_utf8_lossy(&output.stderr));

    // Output file should exist and contain diff content
    let file_content = std::fs::read_to_string(output_path)
        .expect("Output file should exist and be readable");
    assert!(file_content.contains("4840D6"),
        "Output file should contain ICAO: got '{}'", file_content);
    assert!(file_content.contains("DF11"),
        "Output file should show DF11 (right-only): got '{}'", file_content);
    assert!(file_content.contains("Matched:"),
        "Output file should contain summary: got '{}'", file_content);
...
fn test_compare_file_live() {
    ...
    live_buf.push(0x1a); live_buf.push(0x33);
    ...
    assert!(output.status.success(),
        ...
        String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Matched: 1"), "Should show matched count: got '{}'", stdout);
